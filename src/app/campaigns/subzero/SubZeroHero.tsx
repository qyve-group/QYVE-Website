...
interface SubZeroHeroProps {
  price: number;
  previousPrice: number;
  savings: number;
}

const SubZeroHero = ({ price, previousPrice, savings }: SubZeroHeroProps) => {
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-[#0a1f3d] via-[#0d3d5c] to-[#1a5a7a]">
      ...
      <div className="relative min-[1010px]:hidden">
        ...
        {/* Strong gradient overlay for mobile text visibility */}
        <div className="absolute inset-0 bg-gradient-to-t from-[#0a1f3d] via-[#0a1f3d]/60 to-transparent" />
      </div>
          ...
          <div className="flex w-full flex-col items-center space-y-3 text-center sm:space-y-4">
            ...
            <div className="flex flex-col items-center gap-3 sm:flex-row sm:gap-4">
              <Link
                ...
                className="inline-block rounded-full bg-[#4FD1C5] px-8 py-3 text-sm font-bold text-[#0a1f3d] shadow-lg transition-all hover:scale-105 hover:bg-white sm:px-10 sm:py-3.5 sm:text-base"
              >
                Buy Now
              </Link>
              <Link
                href="#features"
                className="inline-block rounded-full border-2 border-white/50 bg-black/20 px-6 py-2.5 text-sm font-semibold text-white backdrop-blur-sm transition-all hover:border-white hover:bg-white/20 sm:px-8 sm:py-3"
              >
                See Tech Breakdown
              </Link>
            </div>

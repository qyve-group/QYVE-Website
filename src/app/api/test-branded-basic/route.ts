...
export async function POST(req: Request) {
  try {
    ...
    console.log('✅ Basic branded email sent successfully:', info.messageId);
    ...
    return NextResponse.json({
      ...
      branding: 'QYVE Brand Colors & Logo',
      timestamp: new Date().toISOString()
    });
    
  } catch (error: any) {
    console.error('❌ Basic branded email test failed:', error);
    return NextResponse.json({ 
      success: false, 
      message: `Basic branded email test failed: ${error.message}` 
    }, { status: 500 });
  }
}

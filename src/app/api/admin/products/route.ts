...
async function verifyAdminAuth(): Promise<{
  ...
  try {
    ...
    if (ADMIN_EMAILS.length > 0 && !ADMIN_EMAILS.includes(userEmail)) {
      return {
        authorized: false,
        error: `Forbidden - ${userEmail} not in admin list`,
      };
    }
...
export async function GET() {
  ...
  if (!auth.authorized) {
    console.log('Admin auth - unauthorized:', auth.error);
  ...
  try {
    ...
    if (productColors && productColors.length > 0) {
      return NextResponse.json(
        {
          error:
            'Cannot delete product with existing variants. Please remove all color variants first.',
        },
        { status: 400 },
      );
    }
    ...
    if (error) {
      ...
      if (error.code === '23503') {
        return NextResponse.json(
          {
            error:
              'Cannot delete product. It has related data (orders, variants, etc.).',
          },
          { status: 400 },
        );
      }

...
export const getShippingStepIndex = (orderStatus: string, shippingStatus: string): number => {
  ...
  const exactMatch = orderStatusMappings.find(
    mapping => mapping.orderStatus.toLowerCase() === cleanOrderStatus && 
...
export const getStatusDescription = (orderStatus: string, shippingStatus: string): string => {
  // Handle null/undefined values
  if (!orderStatus || !shippingStatus) {
  ...
  const mapping = orderStatusMappings.find(
    m => m.orderStatus.toLowerCase() === cleanOrderStatus && 
...
export const updateOrderStatus = async (
  ...
  const { data, error } = await supabaseClient
    ...
    .update({
      ...
      ...(newShippingStatus === 'delivered' && { delivered_at: new Date().toISOString() })
    })
    .eq('id', orderId)
    .select();
